...
| M8 | LLM query builder (stretch)                        | split | in-progress | mac/m8-A-ask (merged 3ef7de7) | **(A) DONE on main:** `blpremote_client.llm.ask(host, prompt)` — claude-haiku-4-5 + `messages.parse()` w/ Pydantic `output_format`; two-block system msg with `cache_control: ephemeral` on schema half; auth stripped from LLM context (`_PlanWithoutAuth` → ask() injects `host._get_token()` post-parse); hard-rule refusal of trade-execution prompts in system prompt; schema via M2(e) cache; key resolution kwarg>env>~/.blpremote/anthropic.json; lazy `anthropic` import. 18 tests added, 139/139 client tests pass. **(B) next:** integration verify against win's BBG + tiny CLI demo. NL -> validated IR; client-side confirm. |
