...
| M9 | Desktop UI per side (IBKR-style) + README rewrite  | split | idle     |        | Krishna 2026-05-08: wants double-click `[Connect]` on each OS, server status LED, ngrok URL auto-piped, NL→IR→Run pane. Plan locked at `docs/M9_UI_PLAN.md`. Tier B (Tkinter + launchers + setup.sh) is the first attack ~5-6h; Tier A (.exe / .app via PyInstaller / py2app) is a follow-up PR if B isn't enough. Bundle README rewrite into the same PR — current README pre-dates M1-M8 entirely. |
