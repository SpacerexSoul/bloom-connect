...
| M4 | Request cache + structured logging + /metrics + JSONL audit | mac   | in-progress | merged via main | **(A)** structured JSON logging + JSONL audit + ir_hash/result_hash + cache_hit field (2baf332). **(B)** Prometheus /metrics + route-template label fix (eb46c1d, 655ca0a). **(C)** LRU+TTL request cache keyed by ir_hash, hit/miss counters (c9fc9bf — local smoke green, live verify pending). **(D)** validator IR_UNVERIFIED_SERVICE warning + interval-required test — pending. M2 follow-ups absorbed into (D). |
