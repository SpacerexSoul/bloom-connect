...
| M2 | Generalize IR for all request types + schema cache | split | in-progress | merged via main | Contract LOCKED at `docs/M2_IR_CONTRACT.md` r2. **(a)** mac collect_response + warnings + 1.1 (9958235). **(b)** win BLP_FIELD_<CATEGORY> (8c076d1). **(c1)** win NormalizedMessage dispatcher (88a5711). **(c2)** win IntradayBar+Tick normalisers (11eb915). **(c3)** win FieldInfo+Schema normalisers (a62face). All cherry-picked / merged to main and live-verified. **(e)** mac bar+tick client funcs shipped on `mac/m2-client-funcs` (e957540), unmerged — adding history/field-info/schema funcs after (d). **(d)** win, idle (next-up after M6 per win's preference). |
