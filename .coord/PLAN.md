...
| M9 | Desktop UI per side (IBKR-style) + README rewrite  | split | review   | mac/m9-client-ui (436ee18) + win/m9-server-ui (82694cc) | Both branches ready for merge. **mac side**: `blpremote_client.ui` module + `tools/blpremote-client-ui.py` entry + `Connect.command` launcher + setup.sh + full README rewrite; 17 controller tests, 165/165 client suite. **win side**: chunks (a)+(b)+(c)+(d) — skeleton, status polling, buttons wired (setup.ps1 Popen + Stop-Process + Send-URL via coord), START_SERVER_UI.bat launcher (pythonw, venv-aware); 25 controller tests, 180/180 server suite. LED palette + button-gating + structural symmetry locked. Live-verified end-to-end via [Send URL to Mac] coord round-trip 2026-05-12T18:17Z. |
