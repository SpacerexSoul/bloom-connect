...
| M8 | LLM query builder (stretch)                        | split | review   | mac/m8-A-ask (3ef7de7) + mac/m8-B-cli (2906a16) | **(A) on main:** `blpremote_client.llm.ask(host, prompt)` — claude-haiku-4-5 + `messages.parse()` w/ Pydantic `output_format`; two-block system msg with `cache_control: ephemeral` on schema half; auth stripped from LLM context (`_PlanWithoutAuth` → ask() injects `host._get_token()` post-parse); hard-rule refusal of trade-execution prompts in system prompt; schema via M2(e) cache; key resolution kwarg>env>~/.blpremote/anthropic.json; lazy `anthropic` import. Win structurally verified 2026-05-08T16:50Z (138+1 skipped tests pass on his side). **(B) on main:** `blpremote-ask` CLI (argparse, --service/--yes/--dry-run/--json/--model), [llm] + [polars] extras formalised in pyproject, full M8_LLM_GUIDE.md. 8 CLI tests added. 147/147 client tests pass. **Awaits:** Krishna-issued ANTHROPIC_API_KEY for win to do live `ask(host, 'AAPL last price') -> host.execute(plan)` end-to-end against BBG; flips to `done` on that. |
