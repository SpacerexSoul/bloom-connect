...
| M8 | LLM query builder (stretch)                        | split | review   | mac/m8-A-ask (3ef7de7) + mac/m8-B-cli (2906a16) + mac/m8-C-openrouter-pivot (c0f0c81) | **(A) ask() core, (B) blpremote-ask CLI, (C) PIVOT to OpenRouter:** all on main. Per Krishna 2026-05-08: ditched anthropic-direct for OpenRouter so we can run cheap models (default `deepseek/deepseek-chat` ~$0.14/$0.28 per 1M, ≈ 50× cheaper than claude-haiku-4-5). OpenAI SDK + `client.beta.chat.completions.parse()` for Pydantic-validated structured output; flip `model=` to route to any OpenRouter-exposed model. Auth-isolation invariant intact (`_PlanWithoutAuth` no auth field, post-parse injection). Safety prompt now spells out BBG mnemonics (PX_LAST not LAST_PRICE) so cheap models don't drift. Key resolution: kwarg>OPENROUTER_API_KEY env>~/.blpremote/openrouter.json. [llm] extra now anchors on `openai>=1.50`. Live-verified 2026-05-08T16:5xZ: deepseek/deepseek-chat against the full M2 schema returns clean 7-op refdata plan, 673/255 tokens. 148/148 mac client tests pass. **Awaits:** win pulls main, swaps install to `[llm]` (openai), gets OpenRouter key from Krishna, runs `blpremote-ask --yes "AAPL last price"` end-to-end against BBG. |
