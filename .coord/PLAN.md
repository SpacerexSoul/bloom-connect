...
| M10| First-run onboarding wizard + ngrok bake-in        | split | idle     |        | Krishna 2026-05-12: ideal flow is "download bloom-connect, pick host or client, wizard handles the rest; Bloomberg is the only manual prereq". Plan locked at `docs/M10_ONBOARDING_PLAN.md`. Tier B (Welcome.command/Welcome.bat + tools/welcome.py Tkinter wizard + enhanced setup.ps1 with ngrok auto-download + JWT auto-gen + pairing-code exchange) is first attack ~6-8h split; Tier A (.exe/.app via PyInstaller/py2app) is M11 territory if needed. Pairing-code design = base64-JSON {url, user, pass}, plaintext credentials with documented trust model. |
