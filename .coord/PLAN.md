...
| M10| Real installer + Settings panel + onboarding wizard | split | in-progress |       | Krishna 2026-05-12: locked Tier A (real `.exe` / `.app` with icon, not script double-clicks) + all env vars editable through UI. **Shipped**: ⚙ Settings dialog in mac client UI replaces JSON-by-hand (`save_settings()` controller method, 6 new tests, 23/23 UI suite); `scripts/build_mac_app.sh` produces a clean 40 MB `Bloomberg Remote.app` + 20 MB `.dmg` installer (was 2 GB before a clean-venv build); `scripts/build_win_exe.ps1` is the matching spec for win to run on Windows (PyInstaller + Inno Setup). Running `.app` verified — macOS menu bar shows "Bloomberg Remote File Edit View Window Help", proving native-app integration (`docs/screenshots/m9-mac-app-running.png`). **Remaining**: win runs build_win_exe.ps1 on a real box and ships the `.exe` installer; first-launch auto-open of Settings if identity.json empty; ngrok auto-download in setup.ps1; pairing-code exchange. Full plan at `docs/M10_ONBOARDING_PLAN.md`. |
