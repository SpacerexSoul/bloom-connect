...
| M7 | pandas/polars client surface                       | mac   | done     | merged via main (c7c2586) | `blpremote_client.dataframes`: pd_/pl_ history/bars/ticks. Optional deps via lazy import; ImportError with install hint if missing. pd_history returns wide MultiIndex; pl_history returns long form. Live-verified both sides 2026-05-08: pd_bars 1 row OHLC=292, pd_history (5,2) wide, pl_history (10,4) long, pl_ticks (619,4) dense. 271/271 mac, 275/275 win tests pass. |
| M9 | Desktop UI per side (IBKR-style) + README rewrite  | split | done     | merged via main (5e6345d) | Both branches merged 2026-05-12. **mac**: `blpremote_client.ui` + `Connect.command` + `setup.sh` + full README rewrite; 17 UI tests + 8 IR-fixer tests, 173/173 client suite. **win**: chunks (a)+(b)+(c)+(d) — skeleton, status polling, buttons wired (setup.ps1 Popen + child-process cleanup + Send-URL via coord), `START_SERVER_UI.bat` launcher; 25 UI tests, 175/175 server suite. **Live end-to-end verified 2026-05-12T19:39Z**: drove client UI via real Tkinter Run button → ngrok → BBG → real `PX_LAST: 294` for AAPL returned in 277ms server-side. IR-fixer (`_normalise_array_paths` in llm.py) deterministically converts `set` → `append` for known array paths (securities/fields/eventTypes/overrides/ids), closing the cheap-model drift failure mode surfaced during live verify. Click-through screenshots in `docs/screenshots/m9-live-{01..05}-*.png`. |
| M10.5| UI-driven first-run + Reset button (no PowerShell ever) | win | done | merged via main (d7133f5) | Krishna 2026-05-12: extension of M10 to remove the one-time PowerShell step. Win shipped on `win/m10.5-first-run-no-ps`: 5 pure helpers (count_users, is_first_run, create_user_inline, set_ngrok_authtoken_inline, reset_install_state), controller auto-opens Settings on first-run detect, Settings dialog conditionally grows first-user + password fields when count_users==0, Save cascades missing state into existence in one click (auto-gen JWT + create_user_inline + auto-emit pairing code), [Reset...] bottom-left with confirm modal + timestamped backup dir + ngrok-authtoken preserved. 16 new tests, 51/51 in server UI suite. Verified on Mac. After this, first-time Windows setup is one Settings dialog away from a running server. |
| M10| Real installer + Settings panel + onboarding wizard | split | done     | merged via main (b4f75c0 + later) | Krishna 2026-05-12: locked dual-track (.exe/.app for personal; setup.ps1/setup.sh for EDR'd boxes) + all env vars editable through UI. **Shipped end-to-end**: ⚙ Settings dialog in both client + server UIs; pairing-code encode/decode (PowerShell host emit + Python client paste); first-launch auto-open Settings; `Bloomberg Remote.app` (40 MB) + `Bloomberg Remote.dmg` (20 MB) installer with the Bc-on-black icon (orange B + green c, translucent OHLC candles, 8 ticker chips); Inno-Setup-wrapped `.exe` via `build_win_exe.ps1` (5 PS5-quirk fixes + conda DLL workaround); `setup.ps1` chunks (a)+(b) ngrok auto-DL + authtoken prompt, (c) JWT secret auto-gen with user-only ACL, (d) first-user prompt via env-var-safe SecureString, (e) pairing-code emit. **Joint fresh-install verify (item 6, ii-track) DONE 2026-05-12T21:03Z**: wiped `~/.blpremote/` → launched `.app` → Settings auto-opened → pairing-code paste from win → Save → Connect → green LED + Connected · 0.1.0 → Ask "AAPL last price" → Run → `status: ok`, `server_timing_ms: 275`, real `PX_LAST: 294` for AAPL came back through the live BBG round-trip. Screenshots at `docs/screenshots/m10-fresh-install-{01..06}-*.png`. Optional follow-up (i): Krishna walks `Read-Host -AsSecureString` prompts at his Windows keyboard whenever convenient. |
| M8 | LLM query builder (stretch)                        | split | done     | mac/m8-A-ask (3ef7de7) + mac/m8-B-cli (2906a16) + mac/m8-C-openrouter-pivot (c0f0c81) | **(A) ask() core, (B) blpremote-ask CLI, (C) PIVOT to OpenRouter:** all on main. Per Krishna 2026-05-08: ditched anthropic-direct for OpenRouter so we can run cheap models (default `deepseek/deepseek-chat` ~$0.14/$0.28 per 1M, ≈ 50× cheaper than claude-haiku-4-5). OpenAI SDK + `client.beta.chat.completions.parse()` for Pydantic-validated structured output. Auth-isolation invariant intact (`_PlanWithoutAuth` no auth field, post-parse injection). Safety prompt spells out BBG mnemonics (PX_LAST not LAST_PRICE). Key resolution: kwarg>OPENROUTER_API_KEY env>~/.blpremote/openrouter.json. [llm] extra anchors on `openai>=1.50`. **Live-verified end-to-end 2026-05-08T17:28Z by win**: all 6 steps pass — `--dry-run "AAPL last price"` returns 7-op refdata plan w/ explain "Fetch AAPL's last price (PX_LAST) via ReferenceDataRequest", `--yes` happy path executes against BBG, refusal on "buy 100 AAPL", `--service //blp/apiflds`, `--json`. Key written to ~/.blpremote/openrouter.json on win box, never echoed back on coord. 9/9 milestones complete. |
