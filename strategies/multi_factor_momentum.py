...
class MultiFactorConfig:
    ...
    excluded_news_keywords: list = field(
        default_factory=lambda: ["fraud", "investigation", "SEC", "lawsuit", "recall", "downgrade"]
    )
...
class MultiFactorMomentumStrategy:
    ...
    def fetch_universe(self, host: "RemoteHost") -> list[str]:
        ...
        print("[2/7] Fetching reference data for filtering...")
        ...
        print("[3/7] Applying universe filters...")
    ...
    def calculate_momentum(self, stock: StockData) -> float:
        ...
        returns = (
            np.diff(np.log(prices[-lookback:-skip]))
            if skip > 0
            else np.diff(np.log(prices[-lookback:]))
        )
        if len(returns) == 0:
            return np.nan
    ...
    def calculate_factor_scores(self) -> list[FactorScores]:
        ...
        print("[6/7] Calculating factor scores...")
        ...
        for stock in self.universe:
            ...
            if stock.volatility_60d > 0:
                if (
                    stock.volatility_20d / stock.volatility_60d
                    > self.config.vol_threshold_multiplier
                ):
        ...
        for score in self.factor_scores:
            ...
            score.passes_all_filters = (
                score.passes_technical
                and score.passes_sentiment
                and score.passes_earnings_blackout
                and score.passes_news_volume
                and not score.volatility_spike
            )
    ...
    def generate_signals(self) -> dict[str, float]:
        ...
        """
        print("[7/7] Generating trading signals...")
        ...
        if self.vix >= self.config.vix_stop_trading_threshold:
            ...
            print("      ⚠️ VIX STOP: All signals set to HOLD")
            for score in self.factor_scores:
        ...
        if self.vix >= self.config.vix_reduce_50_threshold:
            vix_scale = 0.5
            self.alerts.append(
                (AlertLevel.WARNING, f"VIX ({self.vix:.1f}): Reducing positions by 50%")
            )
        elif self.vix >= self.config.vix_reduce_25_threshold:
            vix_scale = 0.75
            self.alerts.append(
                (AlertLevel.INFO, f"VIX ({self.vix:.1f}): Reducing positions by 25%")
            )
    ...
    def compute_positions(
        ...
        sorted_signals = sorted(
            [(t, s) for t, s in signals.items() if s > 0], key=lambda x: x[1], reverse=True
        )
    ...
    def calculate_trailing_stop(
        self, entry_price: float, highest_price: float, atr: float
    ) -> tuple[float, bool]:
        """
    ...
    def calculate_profit_targets(self, entry_price: float, atr: float) -> tuple[float, float]:
    ...
    def check_daily_loss_limits(self, daily_pnl_pct: float) -> str:
        ...
        if daily_pnl_pct <= -self.config.daily_loss_exit_pct:
            ...
            self.alerts.append(
                (AlertLevel.CRITICAL, f"Daily loss {daily_pnl_pct:.1%}: Exit weakest positions")
            )
            return "reduce"
        elif daily_pnl_pct <= -self.config.daily_loss_warning_pct:
            self.alerts.append(
                (AlertLevel.WARNING, f"Daily loss {daily_pnl_pct:.1%}: Stop new buys")
            )
            return "warning"
        return "normal"
    ...
    def check_drawdown_limits(self, drawdown_pct: float) -> str:
        ...
        if drawdown_pct >= self.config.drawdown_hard_stop_pct:
            ...
            self.alerts.append(
                (AlertLevel.EMERGENCY, f"Drawdown {drawdown_pct:.1%}: EXIT ALL POSITIONS")
            )
            return "exit"
        elif drawdown_pct >= self.config.drawdown_reduce_50_pct:
            self.alerts.append(
                (AlertLevel.CRITICAL, f"Drawdown {drawdown_pct:.1%}: Reduce positions by 50%")
            )
            return "reduce_50"
        elif drawdown_pct >= self.config.drawdown_reduce_25_pct:
            self.alerts.append(
                (AlertLevel.WARNING, f"Drawdown {drawdown_pct:.1%}: Reduce positions by 25%")
            )
            return "reduce_25"
        return "normal"
                ...
                if score.ticker == ticker:
                    for stock in self.universe:
                        if stock.bloomberg_ticker == ticker:
                            ...
                            print(
                                f"{score.rank:<5} {ticker:<20} {signal:>+8.3f} "
